...
1. КОНКРЕТНІ факти: ХТО прийняв рішення, ЩО саме змінилось, КОЛИ, ДЛЯ КОГО.
   ...
   Приклад ХОРОШОГО посту: "За даними CNN Travel, Італія скасовує вимогу
   заповнювати форму PLF для туристів з країн, що не входять до ЄС, з 1 березня."
...
=== ПРАВИЛА ===
1. ТІЛЬКИ українською мовою.
2. Пиши лише загальновідомі факти — висоту гір, глибину озер, відомі назви, історичні дати.
   НЕ вигадуй конкретних цифр (відстані, ціни, кількості) якщо не впевнений.
...
- 2-4 емодзі
- Описовий стиль: атмосфера, краса, враження — тут джерела не потрібні,
  пиши як від мандрівника який ділиться враженнями
- Загальновідомі факти ОК (столиця, відома пам'ятка, історична дата)
- В кінці: ненавʼязливо згадай www.im-in.net
...
async def generate_auto_reply(
    ...
    system = (
        ...
        "- Відповідай дружньо та ЗМІСТОВНО. Ніколи не відповідай просто 'дякую за повідомлення' — "
        "це не корисна відповідь. Кожна відповідь ПОВИННА містити конкретну інформацію.\n"
        "- КОЖНУ відповідь завершуй одним із цих закликів (обирай доречний):\n"
        "  • Слідкуйте за нами в соцмережах: Telegram @iminapp_bot\n"
        "  • Більше інформації на сайті www.im-in.net\n"
        "  • Надішліть запит через форму на сайті www.im-in.net\n"
        "  • Залишайтесь на зв'язку — незабаром багато цікавого!\n"
        ...
        "- На привітання ('привіт', 'hello', 'hi') — представся: 'Привіт! Я чат-бот додатку I'M IN "
        "для мандрівників. Можу розповісти про можливості додатку, допомогти з питаннями або "
        "підказати як з нами зв'язатися. Що вас цікавить?'\n"
        "- На загальне повідомлення без конкретного питання — розкажи коротко що таке I'M IN "
        "(додаток для мандрівників з картою подій, фото/відео, спілкуванням) "
        "та запропонуй поставити конкретне питання або відвідати www.im-in.net.\n"
        "- Якщо повідомлення — спам або нерелевантне, класифікуй як spam, "
        "але все одно дай коротку відповідь про додаток.\n"
