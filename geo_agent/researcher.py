...
Поверни JSON:
{
  ...
  "places": [{"name": "Назва", "type": "музей/пам'ятка/ресторан/готель/парк/церква/магазин", "description": "опис", "url": "посилання або null"}],
...
5. type у places — пиши УКРАЇНСЬКОЮ (музей, парк, церква, пам'ятка, готель, ресторан тощо).
6. НЕ ВИГАДУЙ новини! news — тільки реальні факти з наданих даних. Якщо немає — поверни news: [].
7. Для України: аеропорти ЗАКРИТІ з лютого 2022 через війну. НЕ пиши про авіасполучення.
8. Поверни ТІЛЬКИ JSON."""
...
Поверни JSON:
{
  ...
  "places": [{"name": "Назва", "type": "музей/парк/церква/пам'ятка/площа", "description": "опис", "url": "або null"}],
  "news": [{"title": "Заголовок", "description": "Опис", "source": "джерело"}]
}
...
3. type у places — пиши УКРАЇНСЬКОЮ (музей, парк, церква, площа, вулиця тощо).
4. НЕ ВИГАДУЙ новини! news — тільки реальні факти. Якщо немає — поверни news: [].
5. Для України: аеропорти ЗАКРИТІ з лютого 2022. НЕ пиши про авіасполучення.
...
Поверни JSON:
{
  ...
  "history": [{"period": "рік/період", "description": "ключові моменти історії міста — мін 5 записів від заснування до сучасності"}],
  "places": [{"name": "Назва", "type": "музей/парк/собор/площа/район/ринок", "description": "опис", "url": "або null"}],
  "news": [{"title": "Заголовок", "description": "Новини міста", "source": "джерело"}],
  "regions": [{"name": "Назва району міста", "type": "district", "description": "коротко про район"}]
}

КРИТИЧНО:
1. Пиши ТІЛЬКИ про вказане місто!
2. location_name — назва з верифікації, НЕ змінюй!
3. type у places — пиши УКРАЇНСЬКОЮ (музей, парк, собор, площа, район, ринок тощо).
4. НЕ ВИГАДУЙ новини! news — тільки реальні факти. Якщо немає — поверни news: [].
5. Для українських міст: аеропорти ЗАКРИТІ з лютого 2022. НЕ пиши про авіасполучення, відкриття рейсів.
6. history: детальна хронологія від заснування до сучасності. Мінімум 5 записів.
7. regions: основні райони міста з описом.
8. Тільки факти. Поверни ТІЛЬКИ JSON."""
...
Поверни JSON:
{
  ...
  "history": [{"period": "рік/період", "description": "ключові моменти історії країни — мін 8 записів з нашої ери"}],
  ...
  "regions": [{"name": "Назва регіону/області", "type": "region", "description": "коротко про регіон, головне місто, туристичні особливості"}]
}

КРИТИЧНО — ЗАБОРОНИ:
1. НЕ ВИГАДУЙ новини! Секція news — ТІЛЬКИ реальні факти (якщо немає — поверни news: []).
2. Для України (UA): аеропорти ЗАКРИТІ з лютого 2022 через війну. НЕ пиши про відкриття рейсів, нових маршрутів, аеропортів. Це заборонено.
3. Для зон конфліктів: перевіряй актуальність інформації. Краще пустий масив news ніж вигадана новина.
4. type у places — пиши УКРАЇНСЬКОЮ (місто, регіон, острів, парк, курорт тощо).
5. history: детальна хронологія з нашої ери. Мінімум 8 записів від давніх часів до сучасності (I ст., V-IX ст., X-XIII ст., XIV-XVI ст., XVII-XVIII ст., XIX ст., XX ст., XXI ст.).
6. regions: перерахуй ВСІ основні регіони/області країни з описом.
7. Тільки факти. Поверни ТІЛЬКИ JSON."""
